...
- [x] **平仄展示**：`build_strains_data.py` 按卷烘焙（服务端对齐渲染行，**39,414 首/95.1%**
  ...
  settings 新增"平仄"开关（默认关，仅卷页），行末缀 ○平●仄◌未定，
  **偶句末位标红示韵脚位** —— 史记没有、唐诗独有的"语法高亮"
...
- [ ] **LLM 精标（远期，另立计划）**：词表法有一词多义误报与覆盖缺口（如 新豐 不在史记词表），
  精标需引入 `volume_md/NNN.tagged.md` Base Copy + shiji-kb 标注铁律与 SKILL 流程；
  典故实体与 shiji wiki 互链已在实体页实现

### Phase 6 — 可视化实验 ✅ 2026-08-31 完成（地图留待坐标数据）
