...
### Phase 7b — P2 同题消歧修正 ✅ 2026-09-01（用户报告《無題》数据错误引出）

- **Bug**：P2 第①轮 (作者,题目) 精确匹配把 tang 侧同题多首**全部**映射到每一个
  同题御定键（李商隐六首《無題》各键都得到 6 个 id）；P9 的三样片包含闸无法
  识别"超集"，于是把六首合抄文本灌进全部六个键。组诗②通道同病（两组
  《無題二首》互串）。
- **修正**：①/②多候选时按"候选首句是否出现在该御定条目原文中"消歧（按出现
  位置排序）；组诗按（分片,连续下标）聚组，组内任一首句命中即整组保留
  （子诗首句可能恰在丢失列中）。
- **波及面**：729 个冗余文本补丁撤销（原文本就完整）、219 个补丁改为正确的
  单诗文本；字数膨胀 +17.5% → **+14.4%**（挤出的正是合抄重复）；
  三百首锚定 327 → **330** 键（相見時難→539-130、昨夜星辰→539-103 等归位）。
- **精标随迁**：17 个受影响键按各自真身重写/新增标注（望岳岱宗/西岳分家、
  野望五律/七律分家、两首隋宮、两首送別、無題家族全体），零失配、lint 全绿。
...
- [ ] **id_map 撞题致正文串号**（2026-09-16 发现，书坊收官时量到）：
  `build_poem_id_map.py` 按题匹配御定条目与 poet.tang 条目，同一诗人写过
  **多组同名组诗**时三组 refs 会指到同一组，`build_clean_corpus.py` 的回补闸
  （`tg_len > yd_len`）又正好放行，于是多条记录正文全同。
  实例：杜甫《遣興五首》三组，218-12/13/14 都被填成全部 15 首（65 段）。
  全库同题同作者同文的重复组 **41 组 / 116 条**（占 43,103 首的 0.27%），
  多为「句」与组诗。查法：
  `SELECT` 全表按 `(title, author, paragraphs)` 分组取 count>1。
  修法：匹配键加入**首句**（或组内序号）消歧，不能只看题。
  ⚠ 重建 `poems.db` 会改已成书所引正文，**须与书坊侧同步**：
  `shufang/books/b20260902091936`（杜工部集）已按上游分首在书内切回，
  语料修好后要复核该书是否回到与语料一致。
